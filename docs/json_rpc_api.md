...
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  ...
  [here](https://blog.conformal.com/bchd-not-your-moms-bitcoin-daemon/) for
  ...
  [bchwallet](https://github.com/bchsuite/bchwallet).
...
[bchrpcclient](https://github.com/bchsuite/bchrpcclient) package.
...
[bchrpcclient](https://github.com/bchsuite/bchrpcclient) package to connect with
...
func main() {
	...
	// exits.
	connCfg := &bchrpcclient.ConnConfig{
		Host:         "localhost:8334",
		Endpoint:     "ws",
		User:         "yourrpcuser",
		Pass:         "yourrpcpass",
	...
	client, err := bchrpcclient.New(connCfg, nil)
	if err != nil {
		log.Fatal(err)
	}
...
func main() {
	...
	// notifications.
	ntfnHandlers := bchrpcclient.NotificationHandlers{
	...
	client, err := bchrpcclient.New(connCfg, &ntfnHandlers)
	if err != nil {
		log.Fatal(err)
	}
