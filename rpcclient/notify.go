...
type notificationState struct {
	...
	notifySpent        map[bchjson.OutPoint]struct{}
}
...
func (s *notificationState) Copy() *notificationState {
	...
	stateCopy.notifySpent = make(map[bchjson.OutPoint]struct{})
...
func newNotificationState() *notificationState {
	return &notificationState{
		...
		notifySpent:    make(map[bchjson.OutPoint]struct{}),
	}
}
...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *bchutil.Tx, details *bchjson.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *bchutil.Tx, details *bchjson.BlockDetails)
	...
	// made to register for the notification and the function is non-nil.
	OnTxAcceptedVerbose func(txDetails *bchjson.TxRawResult)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	switch ntfn.Method {
	// OnBlockConnected
	case bchjson.BlockConnectedNtfnMethod:
	...
	case bchjson.FilteredBlockConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBlockDisconnected
	case bchjson.BlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case bchjson.FilteredBlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRedeemingTx
	case bchjson.RedeemingTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case bchjson.RelevantTxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanFinished
	case bchjson.RescanFinishedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanProgress
	case bchjson.RescanProgressNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAccepted
	case bchjson.TxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAcceptedVerbose
	case bchjson.TxAcceptedVerboseNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBchdConnected
	case bchjson.BchdConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnAccountBalance
	case bchjson.AccountBalanceNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case bchjson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
func parseChainTxNtfnParams(params []json.RawMessage) (*bchutil.Tx,
	*bchjson.BlockDetails, error) {
	...
	// JSON object.
	var block *bchjson.BlockDetails
	if len(params) > 1 {
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*bchjson.TxRawResult,
	error) {
...
func (c *Client) NotifyBlocksAsync() FutureNotifyBlocksResult {
	...
	cmd := bchjson.NewNotifyBlocksCmd()
...
func (c *Client) notifySpentInternal(outpoints []bchjson.OutPoint) FutureNotifySpentResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
	...
	cmd := bchjson.NewNotifySpentCmd(outpoints)
	return c.sendCmd(cmd)
}

// newOutPointFromWire constructs the bchjson representation of a transaction
// outpoint from the wire type.
func newOutPointFromWire(op *wire.OutPoint) bchjson.OutPoint {
	return bchjson.OutPoint{
		Hash:  op.Hash.String(),
		Index: op.Index,
	}
}
...
func (c *Client) NotifySpentAsync(outpoints []*wire.OutPoint) FutureNotifySpentResult {
	...
	ops := make([]bchjson.OutPoint, 0, len(outpoints))
	for _, outpoint := range outpoints {
		ops = append(ops, newOutPointFromWire(outpoint))
	}
	cmd := bchjson.NewNotifySpentCmd(ops)
	return c.sendCmd(cmd)
}
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := bchjson.NewNotifyNewTransactionsCmd(&verbose)
	return c.sendCmd(cmd)
}
...
func (c *Client) notifyReceivedInternal(addresses []string) FutureNotifyReceivedResult {
	...
	cmd := bchjson.NewNotifyReceivedCmd(addresses)
	return c.sendCmd(cmd)
}
...
func (c *Client) NotifyReceivedAsync(addresses []bchutil.Address) FutureNotifyReceivedResult {
	...
	cmd := bchjson.NewNotifyReceivedCmd(addrs)
	return c.sendCmd(cmd)
}
...
func (c *Client) RescanAsync(startBlock *chainhash.Hash,
	...
	cmd := bchjson.NewRescanCmd(startBlockHashStr, addrs, ops, nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	...
	cmd := bchjson.NewRescanCmd(startBlockHashStr, addrs, ops,
		&endBlockHashStr)
	return c.sendCmd(cmd)
}
...
func (c *Client) LoadTxFilterAsync(reload bool, addresses []bchutil.Address,
	...
	outPointObjects := make([]bchjson.OutPoint, len(outPoints))
	for i := range outPoints {
		outPointObjects[i] = bchjson.OutPoint{
			Hash:  outPoints[i].Hash.String(),
			Index: outPoints[i].Index,
		}
	}

	cmd := bchjson.NewLoadTxFilterCmd(reload, addrStrs, outPointObjects)
	return c.sendCmd(cmd)
}
