...
API.  This client has been tested with bchd (https://github.com/bchsuite/bchd),
...
detected by type asserting the error in a *bchjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*bchjson.RPCError); ok {
  		switch jerr.Code {
  		case bchjson.ErrRPCUnimplemented:
