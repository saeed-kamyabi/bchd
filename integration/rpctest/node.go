...
import (
	...
	rpc "github.com/bchsuite/bchd/rpcclient"
...
// arguments returns an array of arguments that be used to launch the bchd
// process.
...
// command returns the exec.Cmd which will be used to start the bchd process.
...
// to the bchd process that is launched via Start().
...
// as the base for the log and data directories for bchd.
...
// start creates a new bchd process, and writes its pid in a file reserved for
...
// stop interrupts the running bchd process process, and waits until it exits
...
// shutdown terminates the running bchd process, and cleans up all
