...
func commandUsage(method string) {
	usage, err := bchjson.MethodUsageText(method)
	if err != nil {
...
func main() {
	...
	method := args[0]
	usageFlags, err := bchjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := bchjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// bchjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(bchjson.Error); ok {
		...
		// The error is not a bchjson.Error and this really should not
	...
	marshalledJSON, err := bchjson.MarshalCmd(1, cmd)
	if err != nil {
