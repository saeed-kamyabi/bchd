...
func (*wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := bchjson.NewBlockConnectedNtfn(block.Hash().String(), block.Height(),
		block.MsgBlock().Header.Timestamp.Unix())
	marshalledJSON, err := bchjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *bchutil.Block) {
	...
	ntfn := bchjson.NewBlockDisconnectedNtfn(block.Hash().String(),
...
func (m *wsNotificationManager) notifyFilteredBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := bchjson.NewFilteredBlockConnectedNtfn(block.Height(),
...
func (*wsNotificationManager) notifyFilteredBlockDisconnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := bchjson.NewFilteredBlockDisconnectedNtfn(block.Height(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *bchutil.Tx) {
	...
	ntfn := bchjson.NewTxAcceptedNtfn(txHashStr, bchutil.Amount(amount).ToBCH())
	marshalledJSON, err := bchjson.MarshalCmd(nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *bchjson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = bchjson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = bchjson.MarshalCmd(nil,
				verboseNtfn)
			if err != nil {
...
// blockDetails creates a BlockDetails struct to include in bchws notifications
// from a block and a transaction's block index.
func blockDetails(block *bchutil.Block, txIndex int) *bchjson.BlockDetails {
	if block == nil {
		return nil
	}
	return &bchjson.BlockDetails{
		Height: block.Height(),
...
func newRedeemingTxNotification(txHex string, index int, block *bchutil.Block) ([]byte, error) {
	...
	ntfn := bchjson.NewRedeemingTxNtfn(txHex, blockDetails(block, index))
	return bchjson.MarshalCmd(nil, ntfn)
}
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	for i, txOut := range tx.MsgTx().TxOut {
		...
		for _, txAddr := range txAddrs {
			...
			ntfn := bchjson.NewRecvTxNtfn(txHex, blockDetails(block,
				tx.Index()))
...
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *bchutil.Tx,
	...
	if len(clientsToNotify) != 0 {
		n := bchjson.NewRelevantTxAcceptedNtfn(txHexString(tx.MsgTx()))
		marshalled, err := bchjson.MarshalCmd(nil, n)
		if err != nil {
...
out:
	for {
		...
		if err != nil {
			...
			jsonErr := &bchjson.RPCError{
				Code:    bchjson.ErrRPCParse.Code,
		...
		switch authCmd, ok := cmd.cmd.(*bchjson.AuthenticateCmd); {
		...
		if !c.isAdmin {
			if _, ok := rpcLimited[request.Method]; !ok {
				jsonErr := &bchjson.RPCError{
					Code:    bchjson.ErrRPCInvalidParams.Code,
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bchjson.HelpCmd)
	if !ok {
		return nil, bchjson.ErrRPCInternal
	}
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bchjson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &bchjson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bchjson.NotifySpentCmd)
	if !ok {
		return nil, bchjson.ErrRPCInternal
	}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bchjson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, bchjson.ErrRPCInternal
	}
...
func handleNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bchjson.NotifyReceivedCmd)
	if !ok {
		return nil, bchjson.ErrRPCInternal
	}
...
func handleStopNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bchjson.StopNotifySpentCmd)
	if !ok {
		return nil, bchjson.ErrRPCInternal
	}
...
func handleStopNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bchjson.StopNotifyReceivedCmd)
	if !ok {
		return nil, bchjson.ErrRPCInternal
	}
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		...
		if err != nil {
			return &bchjson.RPCError{
				Code: bchjson.ErrRPCInvalidAddressOrKey,
...
func deserializeOutpoints(serializedOuts []bchjson.OutPoint) ([]*wire.OutPoint, error) {
...
var ErrRescanReorg = bchjson.RPCError{
	Code:    bchjson.ErrRPCDatabase,
	Message: "Reorganize",
}
	...
	for _, tx := range blk.Transactions() {
		...
		for txOutIdx, txout := range tx.MsgTx().TxOut {
			...
			for _, addr := range addrs {
				...
				ntfn := bchjson.NewRecvTxNtfn(txHex,
...
func handleRescanBlocks(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bchjson.RescanBlocksCmd)
	if !ok {
		return nil, bchjson.ErrRPCInternal
	}
	...
	discoveredData := make([]bchjson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &bchjson.RPCError{
				Code:    bchjson.ErrRPCBlockNotFound,
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, bchjson.RescannedBlock{
...
fetchRange:
	...
	loopHashList:
		for i := range hashList {
			...
			n := bchjson.NewRescanProgressNtfn(hashList[i].String(),
				blk.Height(), blk.MsgBlock().Header.Timestamp.Unix())
			mn, err := bchjson.MarshalCmd(nil, n)
			if err != nil {
	...
	// been sent.
	n := bchjson.NewRescanFinishedNtfn(lastBlockHash.String(),
		lastBlock.Height(),
		lastBlock.MsgBlock().Header.Timestamp.Unix())
	if mn, err := bchjson.MarshalCmd(nil, n); err != nil {
