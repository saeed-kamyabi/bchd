...
const (
	...
	// BchdConnectedNtfnMethod is the method used for notifications when
	// a wallet server is connected to a chain server.
	BchdConnectedNtfnMethod = "bchdconnected"
...
// BchdConnectedNtfn defines the bchdconnected JSON-RPC notification.
type BchdConnectedNtfn struct {
	Connected bool
}

// NewBchdConnectedNtfn returns a new instance which can be used to issue a
// bchdconnected JSON-RPC notification.
func NewBchdConnectedNtfn(connected bool) *BchdConnectedNtfn {
	return &BchdConnectedNtfn{
		Connected: connected,
	}
}
...
func init() {
	...
	MustRegisterCmd(BchdConnectedNtfnMethod, (*BchdConnectedNtfn)(nil), flags)
