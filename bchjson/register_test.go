...
func TestUsageFlagStringer(t *testing.T) {
	...
	tests := []struct {
		in   bchjson.UsageFlag
		want string
	}{
		{0, "0x0"},
		{bchjson.UFWalletOnly, "UFWalletOnly"},
		{bchjson.UFWebsocketOnly, "UFWebsocketOnly"},
		{bchjson.UFNotification, "UFNotification"},
		{bchjson.UFWalletOnly | bchjson.UFWebsocketOnly,
			"UFWalletOnly|UFWebsocketOnly"},
		{bchjson.UFWalletOnly | bchjson.UFWebsocketOnly | (1 << 31),
			"UFWalletOnly|UFWebsocketOnly|0x80000000"},
	}

	// Detect additional usage flags that don't have the stringer added.
	numUsageFlags := 0
	highestUsageFlagBit := bchjson.TstHighestUsageFlagBit
...
func TestRegisterCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			err: bchjson.Error{ErrorCode: bchjson.ErrDuplicateMethod},
		},
		{
			...
			flags: bchjson.TstHighestUsageFlagBit,
			err:   bchjson.Error{ErrorCode: bchjson.ErrInvalidUsageFlags},
		},
		{
			name:   "invalid type",
			method: "registertestcmd",
		...
		},
		{
			...
			err: bchjson.Error{ErrorCode: bchjson.ErrEmbeddedType},
		},
		{
			...
			err: bchjson.Error{ErrorCode: bchjson.ErrUnexportedField},
		},
		{
			...
			err: bchjson.Error{ErrorCode: bchjson.ErrUnsupportedFieldType},
		},
		{
		...
		},
		{
			...
			err: bchjson.Error{ErrorCode: bchjson.ErrNonOptionalField},
		},
		{
			...
			err: bchjson.Error{ErrorCode: bchjson.ErrNonOptionalDefault},
		},
		{
			...
			err: bchjson.Error{ErrorCode: bchjson.ErrMismatchedDefault},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		err := bchjson.RegisterCmd(test.method, test.cmdFunc(),
			test.flags)
...
func TestMustRegisterCmdPanic(t *testing.T) {
	...
	bchjson.MustRegisterCmd("panicme", 0, 0)
}
