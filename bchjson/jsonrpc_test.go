...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if bchjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *bchjson.RPCError {
				return bchjson.NewRPCError(bchjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := bchjson.MarshalResponse(testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := bchjson.NewRequest(nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := bchjson.Error{ErrorCode: bchjson.ErrInvalidType}
	_, err = bchjson.MarshalResponse(make(chan int), nil, nil)
	if jerr, ok := err.(bchjson.Error); !ok || jerr.ErrorCode != wantErr.ErrorCode {
	...
	// can't be marshalled.
	_, err = bchjson.MarshalResponse(1, make(chan int), nil)
...
func TestRPCError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			bchjson.ErrRPCInvalidRequest,
			"-32600: Invalid request",
		},
		{
			bchjson.ErrRPCMethodNotFound,
