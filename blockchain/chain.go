...
// already there.
func dbMaybeStoreBlock(dbTx database.Tx, block *bchutil.Block) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block *bchutil.Block, view *UtxoViewpoint, stxos []spentTxOut) error {
...
type IndexManager interface {
	...
	// main chain.
	ConnectBlock(database.Tx, *bchutil.Block, *UtxoViewpoint) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain.
	DisconnectBlock(database.Tx, *bchutil.Block, *UtxoViewpoint) error
}
