...
func addrToKey(addr bchutil.Address) ([addrKeySize]byte, error) {
	...
	case *bchutil.AddressWitnessScriptHash:
		...
		// push to 20-bytes.
		copy(result[1:], bchutil.Hash160(addr.ScriptAddress()))
		return result, nil
...
// the passed map.
func (idx *AddrIndex) indexBlock(data writeIndexData, block *bchutil.Block, view *blockchain.UtxoViewpoint) {
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) ConnectBlock(dbTx database.Tx, block *bchutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) DisconnectBlock(dbTx database.Tx, block *bchutil.Block, view *blockchain.UtxoViewpoint) error {
	// Build all of the address to transaction mappings in a local map.
	addrsToTxns := make(writeIndexData)
	idx.indexBlock(addrsToTxns, block, view)
